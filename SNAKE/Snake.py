...
pygame.mixer.music.load('data/Звуки/eat_sound.mp3')
SIZE = [800, 800]
...
class Snake:
    ...
    def __init__(self):
        self.image = pygame.image.load('data/Картинки/snake.jpg').convert_alpha()
...
class Apple:
    def __init__(self):
        ...
        self.image = pygame.image.load('data/Картинки/apple.jpg').convert_alpha()
...
def menu():
    global menu, selected, running, best_score, score, SNAKE
    while menu:
...
def game():
    global running, direction, direction1, best_score, score
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
        ...
        snake.movement2()
        if SNAKE[0] in SNAKE[1:] or SNAKE[0][0] > 24 or SNAKE[0][0] < 0 or \
                SNAKE[0][1] > 24 or SNAKE[0][1] < 0:
            if score > best_score:
                best_score = score
            menu()
